...
ncolumns = 20
flags = 0
banner = "VE3OOI Microcontroller PC Interface v0.1b"
...
photo = ImageTk.PhotoImage(
    Image.open("uC_Console_Chip_Icon.png"))
...
def execute():
    ...
    if plot.get():
        if plotTypeString.get() == CONST.TYPE_XY:
            ...
            plt.xticks(np.arange(float(min(x)), float(max(x)),
                       (float(max(x))-float(min(x))) / 10), rotation='vertical')
